...
const socialHref = (type: string, link: string) =>
  type === "mail" ? `mailto:${link}` : link;
...
const Members = () => {
  ...
  useEffect(() => {
    try {
      const data = (window as any).data as Member[];
  ...
  return (
    <div className="page">
      ...
      <section className="section wrap">
        <div className="sec-h">
          <div className="t">
            <span className="path-label">~/team/members</span>
            <h2>Members</h2>
          </div>
          {members.length > 0 && (
            <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--t-mono)", color: "var(--dim)" }}>
          ...
          <div className="team-grid">
            {members.map((m) => (
              <div className="mcard" key={m.name}>
                <div className="ph">
                  <img
                    src={m.image || "/static/main_app/assets/default-member.png"}
                    alt={m.name}
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = "/static/main_app/assets/default-member.png";
                    }}
                  />
                </div>
                <div className="b">
                  <div className="nm">{m.name}</div>
                  {m.role && <div className="role">{m.role.toLowerCase()}</div>}
                  {m.social_medias.length > 0 && (
                    <div className="socials">
                      {m.social_medias.map((s, i) => {
                        const icon = socialIcon(s.social_media);
                        if (!icon) return null;
                        return (
                          <a
                            key={i}
                            href={socialHref(s.social_media, s.social_media_link)}
                            target={s.social_media !== "mail" ? "_blank" : undefined}
                            rel={s.social_media !== "mail" ? "noreferrer" : undefined}
                            aria-label={s.social_media}
                          >
                            {icon}
                          </a>
                        );
